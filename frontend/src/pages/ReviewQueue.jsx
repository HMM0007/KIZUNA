...
import { AlertTriangle, CheckCircle2, ClipboardCheck, Filter, Link2, Loader2, RefreshCw, Search, XCircle } from "lucide-react";
import { createReviewApi, listEncountersApi, listPendingReviewsApi, listReviewsApi } from "../services/apiService";

const REVIEWABLE = new Set(["FOUNDATION_CONCEPT_ONLY", "UNMAPPED"]);
const statusConfig = {
  FOUNDATION_CONCEPT_ONLY: { label: "Foundation only", className: "border-amber-200 bg-amber-50 text-amber-700", icon: AlertTriangle },
  UNMAPPED: { label: "Unmapped", className: "border-slate-200 bg-slate-100 text-slate-600", icon: XCircle },
};

function StatusBadge({ status }) {
  const config = statusConfig[status] || statusConfig.UNMAPPED;
  const Icon = config.icon;
  return <span className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs font-medium ${config.className}`}><Icon size={14} /> {config.label}</span>;
}

function DecisionBadge({ decision }) {
  const config = { APPROVED: "border-emerald-200 bg-emerald-50 text-emerald-700", REJECTED: "border-red-200 bg-red-50 text-red-700", REVIEW: "border-blue-200 bg-blue-50 text-blue-700" };
  const label = { APPROVED: "Approved", REJECTED: "Rejected", REVIEW: "Kept for review" };
  return <span className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[11px] font-medium ${config[decision] || config.REVIEW}`}><CheckCircle2 size={12} /> {label[decision] || decision}</span>;
}
...
function ReviewQueue() {
  ...
  const loadData = async (showRefresh = false) => {
    try {
      ...
      setError(null);
      const [pendingData, encounterData, reviewData] = await Promise.all([
  ...
  const latestReviews = useMemo(() => {
    ...
    reviews.forEach((review) => {
      const encounterId = review.encounter_id ?? review.id;
      if (!map.has(encounterId)) map.set(encounterId, review);
    });
    return map;
  }, [reviews]);

  const pendingItems = useMemo(() => pending.filter((item) => REVIEWABLE.has(item.mapping_class || "UNMAPPED")), [pending]);
  const reviewedItems = useMemo(() => encounters.filter((encounter) => REVIEWABLE.has(encounter.mapping_class || "UNMAPPED") && latestReviews.has(encounter.id)), [encounters, latestReviews]);
  const allReviewItems = useMemo(() => {
    const map = new Map();
    [...pendingItems, ...reviewedItems].forEach((item) => map.set(item.id, item));
    return [...map.values()].sort((a, b) => Number(b.id) - Number(a.id));
  }, [pendingItems, reviewedItems]);

  const sourceItems = filter === "PENDING" ? pendingItems : filter === "REVIEWED" ? reviewedItems : filter === "FOUNDATION" ? allReviewItems.filter((item) => item.mapping_class === "FOUNDATION_CONCEPT_ONLY") : filter === "UNMAPPED" ? allReviewItems.filter((item) => item.mapping_class === "UNMAPPED") : allReviewItems;
  const filteredEncounters = useMemo(() => {
    const query = searchTerm.trim().toLowerCase();
    return sourceItems.filter((encounter) => !query || [encounter.id, encounter.patient_id, encounter.diagnosis, encounter.namaste_code, encounter.namaste_english, encounter.tm2_code, encounter.tm2_term].filter(Boolean).some((value) => String(value).toLowerCase().includes(query)));
  }, [sourceItems, searchTerm]);

  const foundationCount = allReviewItems.filter((item) => item.mapping_class === "FOUNDATION_CONCEPT_ONLY").length;
  const unmappedCount = allReviewItems.filter((item) => item.mapping_class === "UNMAPPED").length;
  ...
  const saveDecision = async (encounter, decision) => {
    try {
      ...
      await createReviewApi({
        ...
        notes: decision === "APPROVED" ? "Reviewer confirmed the terminology relationship for this encounter." : decision === "REJECTED" ? "Reviewer rejected the terminology relationship for this encounter." : "Reviewer retained this encounter for further terminology review.",
      });

      // Approved/rejected records must disappear from Pending immediately.
      if (decision === "APPROVED" || decision === "REJECTED") {
        setPending((current) => current.filter((item) => Number(item.id) !== Number(encounter.id)));
      }
      setSelected(null);
      await loadData(true);
    } catch (err) {
  ...
  if (loading) return <div className="flex min-h-[400px] items-center justify-center"><div className="flex items-center gap-3 text-sm text-slate-500"><Loader2 size={18} className="animate-spin" /> Loading review queue...</div></div>;

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><h1 className="text-xl font-semibold text-slate-900">Review Queue</h1><p className="mt-1 text-sm text-slate-500">Review encounter mappings that cannot safely be auto-confirmed.</p></div>
        <button type="button" onClick={() => loadData(true)} disabled={refreshing} className="inline-flex items-center justify-center gap-2 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50"><RefreshCw size={15} className={refreshing ? "animate-spin" : ""} /> Refresh</button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="rounded-lg border border-slate-200 bg-white p-4"><p className="text-xs text-slate-400">Pending review</p><p className="mt-1 text-2xl font-semibold text-slate-900">{pendingItems.length}</p></div>
        <div className="rounded-lg border border-slate-200 bg-white p-4"><p className="text-xs text-slate-400">Reviewed</p><p className="mt-1 text-2xl font-semibold text-slate-900">{reviewedItems.length}</p></div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white">
        <div className="border-b border-slate-200 p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="divide-y divide-slate-100">
          {filteredEncounters.map((encounter) => {
            ...
            return <button key={encounter.id} type="button" onClick={() => setSelected(encounter)} className="w-full px-5 py-4 text-left transition hover:bg-slate-50"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-medium text-slate-900">{encounter.diagnosis}</p><StatusBadge status={status} />{decision && <DecisionBadge decision={decision.decision || decision.review_decision} />}</div><div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500"><span>Encounter #{encounter.id}</span><span>Patient · {encounter.patient_id}</span><span className="font-mono">NAMASTE · {encounter.namaste_code || "—"}</span>{encounter.tm2_code && <span className="font-mono">ICD-11 TM2 · {encounter.tm2_code}</span>}</div></div><span className="text-xs font-medium text-slate-500">Open review →</span></div></button>;
          })}
        </div>
        {filteredEncounters.length === 0 && <div className="px-5 py-12 text-center"><ClipboardCheck size={24} className="mx-auto text-slate-300" /><p className="mt-3 text-sm font-medium text-slate-700">No review items found</p><p className="mt-1 text-xs text-slate-400">The Pending filter is driven directly by the API.</p></div>}
      </section>

      {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/30 p-4" onClick={() => !savingDecision && setSelected(null)}><div className="w-full max-w-2xl rounded-xl border border-slate-200 bg-white shadow-xl" onClick={(event) => event.stopPropagation()}>
